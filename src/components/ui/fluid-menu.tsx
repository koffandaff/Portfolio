...
export function MenuContainer({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative w-12 h-12" data-expanded={isExpanded}>
      {/* Container for all items */}
      <div className="relative">
        {/* First item - always visible */}
        <div 
          className="relative w-12 h-12 bg-neutral-900 border border-white/10 text-white cursor-pointer rounded-full group will-change-transform z-50 flex items-center justify-center shadow-lg"
        ...
        {childrenArray.slice(1).map((child, index) => (
          <div 
            key={index} 
            className="absolute top-0 right-0 w-48 bg-neutral-900/95 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden will-change-transform"
            style={{
              transform: `translateY(${isExpanded ? (index + 1) * 52 + 8 : 0}px)`,
