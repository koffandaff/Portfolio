...
export function CardStack<T extends CardStackItem>({
  ...
  const cardWidth = windowWidth && windowWidth < 640 ? windowWidth - 40 : propCardWidth;
  ...
  React.useEffect(() => {
    ...
    setActive((a) => {
      const nextActive = wrapIndex(a, len);
      return nextActive === a ? a : nextActive;
    });
  }, [len]);
      ...
      <div className="absolute inset-0">
        {item.imageSrc ? (
          ...
          <img
            src={item.imageSrc}
            alt={item.title}
            className="h-full w-full object-contain bg-neutral-900/40"
            draggable={false}
            loading="eager"
          />
        ) : (
